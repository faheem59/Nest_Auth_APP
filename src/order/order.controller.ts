...
export class OrderController {
    ...
    @Post('/order')
    async createOrder(@Body() createOrderDto: CreateOrderDTO, @CurrentUser() user: User): Promise<Order> {
    ...
    @Get('/:userId')
    async myOrder(@Param('userId') userId: string, @CurrentUser() user: User): Promise<Order[]> {
        userId = user.id
